...
> 写入四态结果、事件订阅 API。
> 契约已落码于 `src/SuperSampler.Abstractions/`；宿主**拿不到**传输层、驱动、调度器、编解码、
> Block 中的任何一个——门面就是全部。
>
> 配套文档：
...
2. **同一 `(transport, unitId)` 不得被两个启用的 Device 声明**，否则重复轮询同一从站（加载期报错 CGV-12）
...
| 2 | `PointValue GetValueDetail(string deviceId, string pointId)` | 完整三元组 `{值, 质量, 时间戳}`；**读实时缓存，不发起任何通讯**，可高频调用 |
| 3 | `Task<WriteResult> SetValueAsync(string deviceId, string pointId, object? value, CancellationToken ct = default)` | 基础写入，走完整写管道：范围校验 → 联锁 → 权限 → 下发 →（`verify=true` 时）回读；无 user 重载以内置身份 `Local` 记审计 |
| 4 | `Task<WriteResult> SetValueAsync(string deviceId, string pointId, object? value, ActingUser user, CancellationToken ct = default)` | 带操作者身份的写入：权限按该用户角色校验，审计按该用户记账（`ActingUser` 由宿主认证后传入） |
| 5 | `Task<AlarmAckResult> AcknowledgeAlarmAsync(string deviceId, string pointId, string alarmId, CancellationToken ct = default)` | 确认报警：清 AckPending → 报警回到可重触发状态 → 发 `AlarmAcknowledgedEvent`；`alarmId` 与 `AlarmRaisedEvent.AlarmId` 一致（缺省 id 时为 `"pointId#type"`） |
...
- 写路径**无生命周期门禁**：未 Start / 已 Stop / 已 Dispose 之后调用写仍会下发（成败由链路状态决定）；
...
| `WriteOutcome` | 含义 | 处置建议 |
|---|---|---|
| `Succeeded` | 写入成功（`verify=true` 的点已通过回读校验） | 正常刷新界面 |
| `Failed` | 明确失败：设备拒绝或通讯错误 | **可安全重试** |
| `Indeterminate` | 不确定：超时或应答异常，**写入可能已生效** | **绝不自动重试**，须回读确认（人工判断后补一条查询） |
| `Rejected` | 被拒绝：范围/联锁/权限校验未通过，**未发出任何通讯** | 提示用户原因，无需重试 |
...
总线实现 `InProcessEventBus`（`SuperSampler.Core.Events`，net46、零外部依赖）实现 `IEventBus`
（契约在 `SuperSampler.Abstractions.Events`）；宿主通过 `SamplerEngine.Bus` 拿到它。
...
| 值 | 行为 | 适用 |
|---|---|---|
| `Queued`（默认） | 进入该订阅者**专属有界队列**，由专属后台泵线程异步投递；慢消费者不拖累采集线程；单订阅内严格保序（FIFO） | 通用；每个 Queued 订阅各占一条后台线程 |
...
| 事件 | 类别 | 说明 |
|---|---|---|
| `PointValueChangedEvent` | Value | 点位值变化（仅在值/质量相对缓存变化时发出；高频，用批量订阅） |
| `AlarmRaisedEvent` / `AlarmClearedEvent` / `AlarmAcknowledgedEvent` | Alarm | 报警激活/清除/确认，均实现 `IAlarmEvent`（含 `AlarmId`/`DeviceId`/`PointId`），一条订阅收齐 |
| `BackoffEnteredEvent` / `BackoffRecoveredEvent` | Device / Connection | 进入退避 / 退避恢复；`BackoffEnteredEvent` 带档位 `Attempt`、`DelayMs`、`NextRetryAt`（下次重试时刻），宿主可直接显示 |
| `PointWrittenEvent` | Write | 写审计事件，**所有写操作（无论成败）都发一条**；含 `User`/`Outcome`/`Message`/`ElapsedMs`/`DidReadback`/`Readback`/`VerifyMismatch` |
| 错误事件族 | 按来源 | 基接口 `IErrorEvent`，按可恢复性分类为 `ITransientError`/`ILinkError`/`IPermanentError`/`IIndeterminateError`/`IPolicyError`（如 `TimeoutError`、`LinkError`、`ProtocolError`、`DeviceExceptionError`、`WriteIndeterminateError` 等） |
...
| 类型 | 说明 |
|---|---|
| `ActingUser(string Name, string RoleId)` | 宿主认证后的操作者。分工：宿主负责认证（登录），框架按配置的角色做授权并写审计；`RoleId` 对应配置 `Users/Role@id` |
| `ModbusArea` | `Coil` / `DiscreteInput` / `InputRegister` / `HoldingRegister`，与配置里点位 `area` 对应 |
...
| `ErrorInfo(Code, Category, Level, Source, MessageKey, Context)` | 错误的稳定标识：`Code`（如 `MODBUS.EXCEPTION.02`）与 `MessageKey` 跨版本稳定；`Source`（Transport/Protocol/Device/Codec/Policy/Config）指明找谁排查；`Context` 携带结构化上下文（设备/链路/从站/功能码/尝试次数/耗时/收发帧…） |
